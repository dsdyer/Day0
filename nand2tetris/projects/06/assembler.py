...
class Parser:
    def __init__(self, input_file):
        self.commands = len(input_file)
        self.currentPosition = 0
        self.currentCommand = input_file[0]
        self.lead_bits = '000'
    def advance(self):
        self.currentPosition += 1
        self.currentCommand = input_file[self.currentPosition]
...
for i, c in enumerate(input_file):
    if x.commandType() == 'C_COMMAND':
        y = Code(x)
        machine_instruction = x.lead_bits + y.comp() + y.dest() + y.jump()
        readable = ' '.join(re.findall('....?', machine_instruction))
        output_file.append(readable)

    if x.commandType() == 'A_COMMAND':
        number = int(re.findall("[^@]+", c)[0])
        machine_instruction = format(number, "016b")
        readable = ' '.join(re.findall('....?', machine_instruction))
        output_file.append(readable)
